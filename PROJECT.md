...
- **Build & Export Pipeline**:
  - `app/500/page.tsx`, `app/error.tsx`, `app/global-error.tsx`: Pure App Router custom 500 error page and error boundaries
  - `scripts/postbuild.js`: Postbuild synchronization guaranteeing `out/500.html`, `out/404.html`, and asset mirroring for static hosting
...
| 8 | Next.js 500 Export Bug Resolution | Pure App Router `app/500/page.tsx` + `scripts/postbuild.js` eliminating Pages router ENOENT failure | M4 | ORIGINAL_REQUEST §R5 |
...
| M4 | Build Integrity (Pure App Router + postbuild), E2E Tests & Git Sync | `app/500/page.tsx`, `scripts/postbuild.js`, `PROJECT.md` root sync, run `node tests/e2e/run_all.js`, `npm.cmd run build`, git commit | M1, M2, M3 | DONE |
...
- `app/500/page.tsx`: App Router component rendering custom 500 error page matching portal dark-emerald aesthetic.
...
- `app/500/` — Custom App Router 500 page
